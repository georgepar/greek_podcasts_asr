...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Apply noise and create new wav folder",
    )
    parser.add_argument(
        "--fg-snrs",
        type=str,
...
def augment_wav_eager(
    ...
    if wav.strip()[-1] != "|":
        _, wav_fname = os.path.split(wav)
        out_wav = os.path.join(output_wav_folder, wav_fname)
        new_wav = (
        ...
        data_lib.RunKaldiCommand(new_wav)
    # Else if the wav is in a pipe
    else:
        raise ValueError("no pipes in eager mode")
    return out_wav
...
def main():
    ...
    for line in tqdm(wav_scp_file):
        toks = line.rstrip().split(" ")
        utt = toks[0]
        wav = " ".join(toks[1:])
        dur = reco2dur[utt]
        if args.output_wav_folder is None:
            new_wav = augment_wav(
                utt,
                wav,
                dur,
            ...
            new_wav = augment_wav_eager(
                args.output_wav_folder,
                utt,
                wav,
                dur,
                fg_snrs,
