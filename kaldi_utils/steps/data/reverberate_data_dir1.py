...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Apply reverb and create new wav folder",
    )
    parser.add_argument(
        "--rir-set-parameters",
        type=str,
...
def generate_reverberated_wav_scp_eager(
    output_wav_folder,
    ...
    for i in range(start_index, num_replicas + 1):
        if num_replicas > 1:
            output_wav_folder_rep = output_wav_folder.rstrip("/") + str(i)
            os.makedirs(output_wav_folder_rep)
        else:
            output_wav_folder_rep = output_wav_folder
            os.makedirs(output_wav_folder_rep, exist_ok=True)
        for recording_id in tqdm(keys):
            ...
            path2wav, wavname = os.path.split(wav_original_pipe)
            wav_corrupted_pipe = os.path.join(output_wav_folder_rep, wavname)
            ...
            if reverberate_opts == "":
                shutil.copyfile(
                    wav_original_pipe, os.path.join(output_wav_folder_rep, wavname)
                )
            else:
                rev_cmd = "wav-reverberate --shift-output={0} {1} {2} {3}".format(
                    shift_output,
                    reverberate_opts,
                    wav_original_pipe,
                    wav_corrupted_pipe,
                )
                data_lib.RunKaldiCommand(rev_cmd)
...
def create_reverberated_copy(
    ...
    if output_wav_folder is not None:
        generate_reverberated_wav_scp_eager(
            output_wav_folder,
            wav_scp,
            durations,
            output_dir,
